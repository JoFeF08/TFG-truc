...
class SB3PPOEvalAgent:
    ...
    def eval_step(self, state):
        ...
        mask[legal] = True
        try:
            # MaskablePPO: aplica la màscara d'accions legals a la inferència.
            action, _ = self.model.predict(
                obs_flat[np.newaxis], action_masks=mask[np.newaxis], deterministic=True,
            )
        except TypeError:
            # Algorismes SB3 sense suport de màscara (PPO, DQN).
