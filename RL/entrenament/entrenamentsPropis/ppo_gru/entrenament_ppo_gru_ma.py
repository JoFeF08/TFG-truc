"""
Entrenament PPO-GRU per mans individuals.
Episodi = una mà. Reward = punts guanyats/perduts en aquella mà (truc + envit), normalitzat per 24.
L'entorn (TrucEnvMa) retorna done=True al final de cada mà.
L'ecosistema original (TrucEnv / entrenament_ppo_gru.py) no es modifica.
"""
import sys
import os
import random
...
def evaluar_contra_random(agent, env_config, device, num_partides=50):
    """Avalua l'agent contra random en partides senceres (entorn original)."""
